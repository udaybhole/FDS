    ...
    { switch(infix[i])
      {
        case '(': push(infix[i]);
                  break;
        case ')': while((entry=pop())!='(')// entry is the Top element 
                  postfix[p++]=entry;
                  break;
        case '+':
        case '-':
        ...
        case '/':
        if(!empty())
          {  prev=prior(infix[i]);//indicates priority of next element
             entry=pop();
         while(prev<=prior(entry))// comparing priorities of infix elemnt and Top of the stack
         {  postfix[p++]=entry;
           if(!empty())
              entry=pop();
           else
